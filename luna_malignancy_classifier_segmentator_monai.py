...
from data_tools.datatools import load_cancer_nodules_datalist, split_data, balance_and_split_data, load_LUNA16_datalist
...
df.to_csv(os.path.join(PRE_PROCESSED_DIRECTORY, "datalist.csv"), index=False)
...
train_data_list = processed_scans[:int(len(processed_scans)*0.8)]
val_data_list = processed_scans[int(len(processed_scans)*0.8):]

# Save val_data_list to csv file
import pandas as pd
df = pd.DataFrame(val_data_list)
df.to_csv(os.path.join(PRE_PROCESSED_DIRECTORY, "val_datalist.csv"), index=False)
...
class NoduleClassifier(torch.nn.Module):
    def __init__(self, model):
        super().__init__()
        self.model = model
        self.fc = torch.nn.Linear(256, 1)
    ...
    def forward(self, x):
        logits_map, hidden_layer = self.model(x)
        return logits_map, self.fc(hidden_layer)


unet_model_with_fc = BasicUNETRWithFC(unet_model).to(device)

model = NoduleClassifier(unet_model_with_fc).to(device)
...
def validation(epoch_iterator_val):
    ...
    with torch.no_grad():
        ...
        mean_auc_val = roc_auc_metric.aggregate()
        dice_metric.reset()
